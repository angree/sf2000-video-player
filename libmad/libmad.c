...
int mad_get_info(void* hMad, int* samplerate, int* channels)
{
	dec_struct* dec = (dec_struct*)hMad;
	if (!dec || dec->synth.pcm.samplerate == 0)
		return 0;  /* Not yet decoded */

	if (samplerate)
		*samplerate = dec->synth.pcm.samplerate;
	if (channels)
		*channels = dec->synth.pcm.channels;
	return 1;
}
